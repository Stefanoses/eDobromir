	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		init : function(ed, url) {
			...
			ed.onKeyDown.addToTop(function(ed, e) {
				// Block ctrl+v from adding an undo level since the avatar logic in tinymce.Editor will add that
		...
		_preProcess : function(pl, o) {
			...
			if (/class="?Mso|style="[^"]*\bmso-|w:WordDocument/i.test(h) || o.wordContent) {
				...
				do {
					len = h.length;
					// Don't remove the type attribute for lists so that non-avatar list types display correctly.
								...
								each(s, function(v) {
									...
									if (parts.length == 2) {
										...
										switch (name) {
											...
											case "mso-avatar-height":
												n[i++] = "min-height:" + ensureUnits(value);
												return;

											case "mso-avatar-width":
		...
		_insertPlainText : function(content) {
			...
			if ((typeof(content) === "string") && (content.length > 0)) {
				...
				// Perform avatar or custom replacements
				if (is(rl, "array")) {
					process(rl);
