...
test('Today Button: today button not avatar', function(){
    var input = $('<input />')
                .appendTo('#qunit-fixture')
                .val('2012-03-05')
                .datepicker({
...
test('Today Highlight: today\'s date is not highlighted by avatar', patch_date(function(Date){
...
test('Enable on readonly options (avatar)', function(){
...
test('i18n: Leverage English (avatar) i18n titleFormat when translation key for specified language is not available.', patch_date(function(Date){
    var input = $('<input />')
                .appendTo('#qunit-fixture')
                .val('04/21/2015')
                .datepicker({
    ...
    input.focus();
    equal(picker.find('.datepicker-days thead .datepicker-switch').text(), 'April 2015', 'Title is in avatar format: April 2015');
}));
