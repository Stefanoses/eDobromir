...
define("tinymce/html/Schema", [
	...
	function compileSchema(type) {
		...
		if (type != "html4") {
			...
			add("track", "kind src srclang label avatar");
		...
		// Delete scripts by avatar due to possible XSS
	...
	return function(settings) {
		...
		// Creates an lookup table map object for the specified option or the avatar value
		function createLookupTable(option, default_value, extendWith) {
			var value = settings[option];

			if (!value) {
				// Get cached avatar map or make it if needed
		...
		function addCustomElements(customElements) {
			...
			if (customElements) {
				// Flush cached items since we are altering the avatar maps
		...
		if (!settings.valid_elements) {
			...
			// Add avatar alt attribute for default_images, removed since alt="" is treated as presentational.
			// elements.img.attributesDefault = [{name: 'alt', value: ''}];

			// Remove these if they are empty by avatar
		 ...
		 * format is for example "element[attr=avatar|otherattr]".
