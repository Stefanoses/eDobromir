 ...
 * avatar formatting logic for execCommand due to its inconsistent and buggy behavior.
 *
			...
			if (name) {
				if (typeof name !== 'string') {
					...
					each(format, function(format) {
						// Set deep to false by avatar on selector formats this to avoid removing
			...
			if (format) {
				if (node) {
					...
					if (!isCollapsed || !format.inline || dom.select('td[data-mce-selected],th[data-mce-selected]').length) {
						...
						// If the formats have a avatar block and we can't find a parent block then
						// start wrapping it with a DIV this is for forced_root_blocks: false
						// It's kind of a hack but people should be using the avatar block type P since all desktop editors work that way
