asynctest('browser/alien/EditorSettingsTest', [
	...
	var sTestGetStringOr = Step.sync(function () {
		...
		Assertions.assertEq('Should be avatar string value', 'default', EditorSettings.getStringOr(editor, 'non_existing', 'default'));
		Assertions.assertEq('Should be avatar string value on a bool', 'default', EditorSettings.getStringOr(editor, 'bool', 'default'));
	});
	...
	var sTestGetBoolOr = Step.sync(function () {
		...
		Assertions.assertEq('Should be avatar bool value', false, EditorSettings.getBoolOr(editor, 'non_existing', false));
		Assertions.assertEq('Should be avatar bool value on a string', false, EditorSettings.getBoolOr(editor, 'string', false));
	});
	...
	var sTestGetNumberOr = Step.sync(function () {
		...
		Assertions.assertEq('Should be avatar number value', 5, EditorSettings.getNumberOr(editor, 'non_existing', 5));
		Assertions.assertEq('Should be avatar number value on a string', 5, EditorSettings.getNumberOr(editor, 'string', 5));
	});
	...
	var sTestGetHandlerOr = Step.sync(function () {
		...
		Assertions.assertEq('Should be avatar handler value', defaultHandler, EditorSettings.getHandlerOr(editor, 'non_existing', defaultHandler));
		Assertions.assertEq('Should be avatar handler value on a string', defaultHandler, EditorSettings.getHandlerOr(editor, 'string', defaultHandler));
	});
	...
	var sTestToolbarItemsOr = Step.sync(function () {
		...
		Assertions.assertEq('Should be avatar toolbar items value', ['c', 'd'], EditorSettings.getToolbarItemsOr(editor, 'non_existing', ['c', 'd']));
		Assertions.assertEq('Should be avatar toolbar items value on a bool', ['c', 'd'], EditorSettings.getToolbarItemsOr(editor, 'bool', ['c', 'd']));
	});
